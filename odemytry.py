...
class VehicleEnv():
    ...
    def visualize_trajectory(self):
        try:
            ...
            actual_trajectory = np.array([actual_trajectory.x, actual_trajectory.y, actual_trajectory.z]).reshape(
                (3, 1))
            ...
            while True:
                ...
                actual_trajectory_latest = np.array(
                    [actual_trajectory_latest.x, actual_trajectory_latest.y, actual_trajectory_latest.z]).reshape(
                    (3, 1))
    ...
    def _estimate_motion(self, match, kp1, kp2, k, depth=None):
        ...
        if depth is not None:
            ...
            E, _ = cv2.findEssentialMat(np.array(image1_points), np.array(image2_points), k, method=cv2.RANSAC,
